...
<div class="relative py-6 select-none">
	...
	<div class={`h-[300px] w-[280px] rounded-2xl border ${border} overflow-hidden flex flex-col items-stretch`}>
		<!-- Base content -->
		<h3 class={`p-4 pb-1 text-xl text-gray-200 font-display font-bold leading-heading ${bg} shrink-0`}>
			{item.title}
		</h3>
		<div class="space-y-3 text-white relative p-4 h-0 grow overflow-hidden">
			{#if item.description}
				<p class="text-gray-400 text-xl font-body leading-body">{item.description}</p>
