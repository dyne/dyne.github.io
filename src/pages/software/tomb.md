...
Tomb generates encrypted storage containers that can be opened only by their associated key files, which are also protected by a password.

A tomb is like a locked folder that can be safely transported and hidden in a filesystem; its keys should be kept separate, for instance storing a tomb file in your computer and the key file in a USB stick.

If you are already familiar, [download the 💾 tar.gz](https://files.dyne.org/tomb) or jump to the [📃 README on Github ](https://github.com/dyne/Tomb)
...
Tomb is a simple shell script, you can just download it and copy it into your path, and it will work.

Tomb is operated from a terminal command line and requires **root access** to the machine or just sudo access to the script.

Once open, the tombs are just like normal folders and can contain
different files, plus they offer advanced functionalities like fast search over contents and filenames, or they can be slammed in an emergency. Keys can be stored on separate media like USB sticks, RFID cards,
on-line SSH servers and Bluetooth devices to secure data transport:
for access to contents, one always needs the tomb file as well as the key and the password, so it can be safer to let them travel separately.

The tomb script takes care of several details to improve user's
behaviour and the security of tombs in everyday usage:
- protects the typing of passwords from keyloggers
- facilitates hiding keys inside images
- facilitates searching inside a tomb for files and contents
- automatically mounts directories in place
- lists open tombs and selectively closes them
- warns the user about free space and last time usage
- ... and even more functionalities listed below!
...
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/CMLJmfjCaGM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
[Tomb on LinuxActionShow 2014 YouTube · Dyne.org 21 minutes, 48 seconds Nov 10, 2014](https://www.youtube.com/watch?v=CMLJmfjCaGM)
